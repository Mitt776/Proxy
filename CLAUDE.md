...
**Слои.** Фронтенд (`frontend/src/App.svelte` — только раскладка и вкладки; вся логика в
`frontend/src/lib/`: `ConnectionPanel`, `ProfilesTab`, `RulesTab` + `RuleEditor` + `ProcessPicker` +
`DomainCheck`, `ConnectionsTab`, `LogsTab`, `SettingsTab`, общий стор `store.ts` и глобальные
стили `ui.css`) → биндинги Wails
