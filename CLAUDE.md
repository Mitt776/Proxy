...
- `lib/mobile/` — мобильная оболочка (2.1.0): `TopBar` (бургер, знак, RU/EN), `Drawer`
  (выдвижное меню пяти разделов с карточкой активного профиля), `ConnectScreenMobile`, `Soon`
  (заглушка ещё не портированного раздела). Отдельные компоненты, а не ветки `{#if}` в
  десктопных: у `ConnectScreen` раскладка построена вокруг трёх колонок, у `TitleBar` задача
  вообще другая — заменить рамку безрамочного окна. Тяжёлое переиспользуется как есть:
  `PowerButton`, `Flag`, `ProfilePicker`, сторы, словари, `ui.css`.
