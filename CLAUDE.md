...
- **VPN обязан объявить системе свой DNS** (`dns_address` у tun-инбаунда → `tun.Options.DNSAddress`
  → `builder.addDnsServer` в `TunnelService.openTun`). Иначе в `dumpsys connectivity` у tun0
  стоит `DnsAddresses: [ ]`, и Android оставляет приложениям резолвер нижней сети — роутера или
  оператора. На заблокированный домен оттуда приезжает подставной адрес (fake-ip домашнего
  роутера с собственным обходом, заглушка провайдера), приложение отправляет его в туннель, а на
  ноде он не значит ничего. Симптом сбивает с толку: туннель жив, `github.com` открывается,
  `facebook.com` рвётся мгновенно — но по настоящему IP (`curl --resolve`) тот же сайт через тот
  же туннель грузится нормально. Поле платформенное: значение задаётся константой
  `tunDNSAddress` в `config/tundns_android.go` (на прочих платформах — `tundns_other.go`, пусто),
  потому что **штатный sing-box поля не знает и отвергает конфиг целиком** («unknown field»), то
  есть на Windows оно сломало бы подключение вовсе. Обе стороны закрыты
  `TestTunDNSAddressBothCores`. Адрес обязан лежать в подсети туннеля (`172.19.0.1/30` — свободен
  только `.2`), иначе пакет к нему не попадёт в tun и перехватывать будет нечего.
