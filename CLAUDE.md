...
- `lib/mobile/` — мобильная оболочка (2.1.0): `TopBar` (бургер, знак, RU/EN), `Drawer`
  ...
  импорта), `SettingsTabMobile`, `AppPicker` (приложения «мимо VPN»), `Soon` (заглушка ещё не
  портированного раздела). Отдельные компоненты, а не ветки `{#if}` в десктопных: у
  ...
  `lib/profiles.ts`, сторы, словари, `ui.css`.
