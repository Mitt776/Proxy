...
// Ядро подключается библиотекой (см. этап 0 плана порта): на Android /dev/net/tun
// без рута недоступен, TUN приходит из VpnService файловым дескриптором внутрь процесса.
replace github.com/sagernet/sing-box => github.com/Leadaxe/sing-box-lx v1.14.0-lx.26

// Общие backend-пакеты живут в корневом модуле; сам корень (Wails) в APK не едет.
replace Proxy => ../

// Форк тянет свои патчи wireguard-go гит-сабмодулем, а в zip Go-модуля сабмодули не
// попадают — без этого replace ломается protocol/masque (он тянет transport/wireguard
// даже без тега with_wireguard). Клон делает scripts\android-deps.ps1, коммит закреплён
// там же; каталог в .gitignore.
